...
///
/// Pass a [logo] widget to render a brand mark before the [appName] text. If
/// [logo] is provided AND [appName] is empty, only the logo renders (use this
/// when your logo already contains the wordmark).
class FlaiTopNavBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  /// Optional brand mark rendered before [appName]. Pass a `CmmdLogo()` (or
  /// any custom widget) to display a logo. When this is non-null and
  /// [appName] is empty, only the logo renders.
  final Widget? logo;
  ...
  Widget build(BuildContext context) {
    ...
    final hasName = appName.isNotEmpty;
      ...
      child: Row(
        children: [
          ...
          if (hasLogo) ...[
            ...
            if (hasName) SizedBox(width: theme.spacing.xs),
          ],
          if (hasName)
            Text(
              appName,
