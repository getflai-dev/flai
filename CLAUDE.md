...
- **Drawer dialog crash (Flutter #106549)** — `showDialog` or `showModalBottomSheet` from within a Scaffold Drawer causes `_dependents.isEmpty` assertion on ANY subsequent `notifyListeners()` rebuild. Debug-only (release strips assert). Fix: never show dialogs from drawer widgets. Signal intent via `VoidCallback`, close drawer first, parent shows dialog from outside. `ChatListItem.onRename` is `VoidCallback` for this reason. (2026-03-27)
- **Maestro clearState vs Keychain** — `clearState` does NOT clear iOS Keychain (where session tokens live). Use `runFlow: when: visible: "Log in"` with `shared_login.yaml` for tests that should handle both fresh and restored sessions. Only use `clearState + clearKeychain` when you specifically need a fresh auth state. (2026-03-27)
